...
public partial class MainPage : ContentPage
{
    ...
    // Botón Conectar a RabbitMQ
    private async void OnConnectClicked(object sender, EventArgs e)
    {
        if (_isConnected) return;

        string ip = Preferences.Get("BrokerIp", "localhost");
        string qPub = Preferences.Get("QueuePub", "cola2");
        string qSub = Preferences.Get("QueueSub", "cola1");

        try
        {
            // Suscribirse al evento de recibir mensaje
    ...
    private async Task ProcessLlmResponse(string inputMessage)
    {
        ...
        Title = "Chat 2 - Equipo PERRO 🐶"; // Restaurar título
    ...
    // Botón Enviar Manual (Para romper el hielo)
